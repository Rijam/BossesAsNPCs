...
namespace BossesAsNPCs.Items.Vanity.EmpressOfLight
{
	...
	public class EoLCostumeLegpiece : VanityBase
	{
		...
		public override void Load()
		{
			if (!Main.dedServ)
			{
				// LegEquipTextureMale = EquipLoader.AddEquipTexture(Mod, (GetType().Namespace + "." + Name).Replace('.', '/') + "_Legs", EquipType.Legs, this, "EoLCostumeLegpiece_Legs");
				LegEquipTextureFemale = EquipLoader.AddEquipTexture(Mod, (GetType().Namespace + "." + Name).Replace('.', '/') + "_FemaleLegs", EquipType.Legs, this, "EoLCostumeLegpiece_FemaleLegs");
			}
		}
