...
namespace BossesAsNPCs.Items.Vanity.IceQueen
{
	...
	public class IQCostumeLegpiece : VanityBase
	{
		...
		public override void Load()
		{
			if (!Main.dedServ)
			{
				// LegEquipTextureMale = EquipLoader.AddEquipTexture(Mod, (GetType().Namespace + "." + Name).Replace('.', '/') + "_Legs", EquipType.Legs, this, "IQCostumeLegpiece_Legs");
				LegEquipTextureFemale = EquipLoader.AddEquipTexture(Mod, (GetType().Namespace + "." + Name).Replace('.', '/') + "_FemaleLegs", EquipType.Legs, this, "IQCostumeLegpiece_FemaleLegs");
			}
		}
