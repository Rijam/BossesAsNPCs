...
namespace BossesAsNPCs.NPCs.TownNPCs
{
	...
	public class WallOfFlesh : ModNPC
	{
		...
		public override void HitEffect(int hitDirection, double damage)
		{
			if (NPC.life <= 0)
			{
				Gore.NewGore(NPC.position, NPC.velocity, ModContent.Find<ModGore>("BossesAsNPCs/WallOfFlesh_Gore_Head").Type, 1f);

				for (int k = 0; k < 2; k++)
				{
					Gore.NewGore(NPC.position, NPC.velocity, ModContent.Find<ModGore>("BossesAsNPCs/WallOfFlesh_Gore_Arm").Type, 1f);
					Gore.NewGore(NPC.position, NPC.velocity * (k + 1), ModContent.Find<ModGore>("BossesAsNPCs/WallOfFlesh_Gore_Flesh").Type, 1f);
				}
			}
		}
