...
namespace BossesAsNPCs.NPCs.TownNPCs
{
	...
	public class WallOfFlesh : ModNPC
	{
		...
		public override void SetDefaults()
		{
			...
			NPC.catchItem = ModContent.GetInstance<BossesAsNPCsConfigServer>().CatchNPCs ? (short)ModContent.ItemType<Items.CaughtWallOfFlesh>() : (short)-1;
		}
		...
		public override void SetBestiary(BestiaryDatabase database, BestiaryEntry bestiaryEntry)
		{
			bestiaryEntry.Info.AddRange(new IBestiaryInfoElement[]
			{
				...
				new FlavorTextBestiaryInfoElement("Love: Guide\nLike: Caverns, Eater of Worlds, Brain of Cthulhu, Tax Collector, Goblin Tinkerer\nDislike: Forest, Queen Bee\nHate: None")
			});
		}
		...
		public override void HitEffect(int hitDirection, double damage)
		{
			if (NPC.life <= 0)
			{
				Gore.NewGore(NPC.GetSource_Death(), NPC.position, NPC.velocity, ModContent.Find<ModGore>("BossesAsNPCs/WallOfFlesh_Gore_Head").Type, 1f);

				for (int k = 0; k < 2; k++)
				{
					Gore.NewGore(NPC.GetSource_Death(), NPC.position, NPC.velocity, ModContent.Find<ModGore>("BossesAsNPCs/WallOfFlesh_Gore_Arm").Type, 1f);
					Gore.NewGore(NPC.GetSource_Death(), NPC.position, NPC.velocity * (k + 1), ModContent.Find<ModGore>("BossesAsNPCs/WallOfFlesh_Gore_Flesh").Type, 1f);
				}
			}
		}
	...
	public class WallOfFleshProfile : ITownNPCProfile
	{
		...
		public Asset<Texture2D> GetTextureNPCShouldUse(NPC npc) => ModContent.Request<Texture2D>("BossesAsNPCs/NPCs/TownNPCs/WallOfFlesh");

		public int GetHeadTextureIndex(NPC npc) => ModContent.GetModHeadSlot("BossesAsNPCs/NPCs/TownNPCs/WallOfFlesh_Head");
	}
}